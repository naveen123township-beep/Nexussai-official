 ...
 * Enhanced to handle automated cross-device queue updates
 */
...
/**
 * ATOMIC PIPELINE: Forces direct push for critical orders
 */
async function forceDirectCloudUtrPush(newUtrRecord) {
    const contextKey = "global_nexus_master_registry";
    try {
        const res = await fetch(`${BASE_CLUSTER_URL}${contextKey}?nocache=${Date.now()}`);
        let serverData = res.ok ? await res.json() : {};
        ...
        // Remove duplicates if the user is resubmitting a corrected item
